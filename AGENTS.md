...
- The v3 eMMC change must remain a narrowly auditable delta from the matched reference DTB. `patch-emmc-dtb.sh` enables `/soc/mmc@4022000`, uses the existing `reg_cldo3`/`reg_cldo1` supplies, sets an 8-bit bus, and caps it at 52 MHz SDR. Do not enable HS200/DDR or change regulator voltages without hardware evidence.
