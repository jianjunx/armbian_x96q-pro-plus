...
set -euo pipefail
baseline=4898a7abe86e08c533ace788604a90ce58401dd2fc63ae42a31a5e8249c7e357
node=/soc/mmc@4021000
pins=/soc/pinctrl@2000000/mmc1-pins
dtb=/boot/dtb/allwinner/sun55i-h728-x96qpro+.dtb
recovery=/boot/h728-wifi-clock40-drive20-v1-recovery
tmp=
...
make_candidate() {
    [[ $(hash "$1") == "$baseline" ]] || die 'DTB is not the pinned 24 MHz / 4-bit baseline.'
    [[ $(fdtget -t i "$1" "$node" max-frequency) == 24000000 ]] || die 'Expected 24 MHz.'
    [[ $(fdtget -t i "$1" "$node" bus-width) == 4 ]] || die 'Expected 4-bit input.'
    [[ $(fdtget -t i "$1" "$pins" drive-strength) == 20 ]] || die 'Expected 20 mA.'
    cp -- "$1" "$2"
    fdtput -t i "$2" "$node" max-frequency 40000000
    [[ $(fdtget -t i "$2" "$node" max-frequency) == 40000000 ]] || die 'Clock patch failed.'
...
[[ $mode == arm || $mode == restore ]] || die 'Usage: bash h728-wifi-clock40-drive20-v1-test.sh arm|restore'
...
if [[ $mode == arm ]]; then
    ...
    make_candidate "$recovery/original.dtb" "$recovery/clock40.dtb"
    sync
    tmp=$(mktemp /boot/dtb/allwinner/.h728-wifi-clock40-drive20.XXXXXX)
    cp -- "$recovery/clock40.dtb" "$tmp"
    cmp -s "$tmp" "$recovery/clock40.dtb" || die 'Staging verification failed.'
    sync
    mv -f -- "$tmp" "$dtb"
    tmp=
    sync
    cmp -s "$dtb" "$recovery/clock40.dtb" || die 'Installed DTB verification failed.'
    echo 'WIFI_CLOCK40_DRIVE20_ARMED. No reboot performed. This is an experiment, not a confirmed fix.'
else
