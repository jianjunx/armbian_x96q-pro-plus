...
recovery=/boot/h728-wifi-drive20-v1-recovery
tmp=
...
make_candidate() {
    ...
    cp -- "$1" "$2"
    fdtput -t i "$2" "$pins" drive-strength 20
    [[ $(fdtget -t i "$2" "$pins" drive-strength) == 20 ]] || die 'Drive-strength patch failed.'
    # Restore the property temporarily: byte equality proves no other delta.
    fdtput -t i "$2" "$pins" drive-strength 40
    cmp -s "$1" "$2" || die 'Unexpected DTB changes beyond the drive-strength property.'
    fdtput -t i "$2" "$pins" drive-strength 20
}
...
[[ $mode == arm || $mode == restore ]] || die 'Usage: bash h728-wifi-drive20-v1-test.sh arm|restore'
...
if [[ $mode == arm ]]; then
    ...
    make_candidate "$recovery/original.dtb" "$recovery/drive20.dtb"
    sync
    tmp=$(mktemp /boot/dtb/allwinner/.h728-wifi-drive20.XXXXXX)
    cp -- "$recovery/drive20.dtb" "$tmp"
    cmp -s "$tmp" "$recovery/drive20.dtb" || die 'Staging verification failed.'
    sync
    mv -f -- "$tmp" "$dtb"
    tmp=
    sync
    cmp -s "$dtb" "$recovery/drive20.dtb" || die 'Installed DTB verification failed.'
    echo 'WIFI_DRIVE20_ARMED. No reboot performed. This is an experiment, not a confirmed fix.'
else
