#!/bin/bash
# Direct 40 MHz -> 24 MHz experiment; recovery returns to verified 20 MHz.
set -euo pipefail
...
recovery=/boot/h728-wifi-clock24-v1-recovery
tmp=
expected20=
cleanup() {
    [[ -z $tmp ]] || rm -f -- "$tmp"
...
install_dtb() {
    local source=$1
    [[ -z $tmp ]] || rm -f -- "$tmp"
    tmp=$(mktemp /boot/dtb/allwinner/.h728-wifi-clock24.XXXXXX)
    cp -- "$source" "$tmp"
...
[[ $mode == arm || $mode == restore ]] || die 'Usage: bash h728-wifi-clock24-test.sh arm|restore'
...
expected20=$(mktemp /boot/dtb/allwinner/.h728-wifi-clock24.XXXXXX)
make_dtb "$clock12_recovery/original.dtb" "$expected20" 20000000

if [[ $mode == arm ]]; then
    [[ $(cat /sys/class/net/end0/carrier 2>/dev/null) == 1 ]] || die 'Connect Ethernet before arming; Wi-Fi may fail at 24 MHz.'
    tmp=$(mktemp /boot/dtb/allwinner/.h728-wifi-clock24.XXXXXX)
    make_dtb "$clock12_recovery/original.dtb" "$tmp" 40000000
    cmp -s "$dtb" "$tmp" || die 'Active DTB is not the expected 40 MHz candidate.'
    [[ ! -e $recovery ]] || die '24 MHz recovery directory already exists; refusing to overwrite it.'
    mkdir "$recovery"
    cp -- "$dtb" "$recovery/previous40.dtb"
    cmp -s "$recovery/previous40.dtb" "$tmp" || die '40 MHz backup verification failed.'
    cp -- "$expected20" "$recovery/clock20.dtb"
    cmp -s "$recovery/clock20.dtb" "$expected20" || die '20 MHz backup verification failed.'
    make_dtb "$clock12_recovery/original.dtb" "$recovery/clock24.dtb" 24000000
    install_dtb "$recovery/clock24.dtb"
    echo 'WIFI_CLOCK24_ARMED. No reboot performed. This remains an experiment.'
else
    [[ -f $recovery/clock20.dtb && -f $recovery/clock24.dtb ]] || die '24 MHz recovery files missing.'
    cmp -s "$recovery/clock20.dtb" "$expected20" || die '20 MHz recovery verification failed.'
    tmp=$(mktemp /boot/dtb/allwinner/.h728-wifi-clock24.XXXXXX)
    make_dtb "$clock12_recovery/original.dtb" "$tmp" 24000000
